...
var specChineseSync = SpecText{
	...
	synopsisText: "将目录或者oss prefixes从源端同步到目的端",
...
`,
    ...
    上进行object拷贝; 用于源端和目的端进行数据同步. 

    sync命令和cp命令不同之处如下: 
    1、当输入--delete选项时,该命令会自动删除目的端存在而源端不存在的object或者移走文件
       如果目的端是oss, 则会删除多余的object; 如果目的端是本地文件, 则会移走多余的文件

       警告: 在没有完全搞清楚sync命令的行为之前, 请慎用--delete

    2、sync强制是recursive遍历文件或者object行为, 所以不用输入-r --recursive
...
`,
    ...
    执行sync命令之后, oss object keys prefix1/a.txt, prefix1/b.txt 被拷贝到
...
var specEnglishSync = SpecText{
	...
	synopsisText: "Sync the directory or oss prefixes from the source to the destination",
...
`,
    ...
    1、When the --delete option is entered, the command will automatically delete objects or remove files 
       that exist on the destination but not on the source.
    ...
    2、Sync is mandatory to traverse files or objects recursively, so there is no need to enter -r or --recursive
    3、When the src is oss://bucket/prefix, sync command will add character '/' after the prefix, 
       but cp command doesn't add
...
--delete
    Indicates that the object or file which exists on destination
    and not exist on src needs to be deleted or backup

--backup-dir
    This option indicates the directory used to back up the destination files needed to be deleted, 
...
`,
    ...
    and local file d.txt is removed to dir backup
...
func (sc *SyncCommand) RunCommand() error {
	...
	if srcURL.IsFileURL() && destURL.IsFileURL() {
		return fmt.Errorf("not support sync between local directories or files")
	}
	...
	// cp command will have the same action when run as sync command
